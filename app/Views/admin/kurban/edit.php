...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 max-w-3xl">
    ...
    <form action="<?= base_url('admin/kurban/update/'.$package['id']) ?>" method="POST" enctype="multipart/form-data">
        <?= csrf_field() ?>
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="name" name="name" value="<?= esc($package['name']) ?>" required placeholder="Contoh: Paket Personal Plus" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select id="type" name="type" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all bg-white" required>
                            <option value="kambing" <?= $package['type'] == 'kambing' ? 'selected' : '' ?>>Kambing / Domba</option>
                            <option value="sapi_patungan" <?= $package['type'] == 'sapi_patungan' ? 'selected' : '' ?>>Sapi Patungan (1/7)</option>
                            <option value="sapi_utuh" <?= $package['type'] == 'sapi_utuh' ? 'selected' : '' ?>>Sapi Utuh</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="1000" id="price" name="price" value="<?= esc($package['price']) ?>" required placeholder="2500000" 
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="weight_range" name="weight_range" value="<?= esc($package['weight_range']) ?>" placeholder="Contoh: Bobot Sapi 250 - 300 KG" 
                           class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" placeholder="Jelaskan spesifikasi atau benefit dari paket ini..." 
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all"><?= esc($package['description']) ?></textarea>
            </div>
            ...
            <div>
                ...
                <div class="space-y-4">
                    <?php if($package['image']): ?>
                        <div class="relative w-40 h-24 rounded-lg overflow-hidden border border-gray-100 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        ...
                        <div class="text-xs text-gray-400 italic font-medium">Atau perbarui URL gambar:</div>
                        <input type="text" name="image" value="<?= esc($package['image']) ?>" placeholder="https://example.com/image.jpg" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all text-sm">
                    </div>
                </div>
            </div>
            ...
            <div class="pt-4 flex justify-end">
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-lg shadow-sm font-medium transition flex items-center">
                    <i class="fa-solid fa-save mr-2"></i> Perbarui Paket Kurban
                </button>
            </div>
        </div>
    </form>
