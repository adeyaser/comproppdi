...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 max-w-3xl">
    ...
    <form action="<?= base_url('admin/rekening/update/'.$account['id']) ?>" method="POST">
        <?= csrf_field() ?>
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="bank_name" name="bank_name" value="<?= esc($account['bank_name']) ?>" required placeholder="Contoh: BSI, Mandiri, BCA" 
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="account_number" name="account_number" value="<?= esc($account['account_number']) ?>" required placeholder="Hanya angka, contoh: 9555555400" 
                           class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="account_name" name="account_name" value="<?= esc($account['account_name']) ?>" required placeholder="Contoh: Yayasan Maziska PPDI" 
                           class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="category" name="category" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all bg-white" required>
                        <option value="zakat" <?= $account['category'] == 'zakat' ? 'selected' : '' ?>>Zakat</option>
                        <option value="infak" <?= $account['category'] == 'infak' ? 'selected' : '' ?>>Infak, Sedekah, Wakaf</option>
                        <option value="kemanusiaan" <?= $account['category'] == 'kemanusiaan' ? 'selected' : '' ?>>Kemanusiaan / Bencana</option>
                        <option value="lainnya" <?= $account['category'] == 'lainnya' ? 'selected' : '' ?>>Lainnya</option>
                    </select>
                </div>
            ...
            <div class="pt-6 border-t border-gray-100 flex justify-end">
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-lg shadow-sm font-medium transition flex items-center">
                    <i class="fa-solid fa-save mr-2"></i> Perbarui Rekening
                </button>
            </div>
        </div>
    </form>
